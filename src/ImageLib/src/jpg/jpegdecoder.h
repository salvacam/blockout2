...
public:
  ...
  virtual int read(uchar *Pbuf, int max_bytes_to_read, bool *Peof_flag)
  {
	...
	bytes_read = (unsigned long)fread(Pbuf, 1, max_bytes_to_read, Pfile);

    if ((int)bytes_read < max_bytes_to_read)
    {
      eof_flag = true;
      *Peof_flag = true;
    }
